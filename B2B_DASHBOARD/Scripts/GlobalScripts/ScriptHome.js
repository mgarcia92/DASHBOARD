            ...
            success: function (data) {
                ...
                function makeInfoWindowEvent(map, infowindow, contentString, marker) {
                    google.maps.event.addListener(marker, 'click', function () {
                        ...
                        calculateRoute(directionsService, directionsDisplay, MyMarker.position, marker.position)
                    });
                }
                */
                // --- DESCOMENTAR Y SUSTITUIR GOOGLE MAP por MAPQUEST
                // L.mapquest.directions().route(options, callback) -> directionsDisplay

/* -------------------- Direcciones de Ventas -------------------------  */
                /*
                function CalculateRouteEvent(map, marker, directionsService, directionsDisplay) {
                ...
                */
                ...
                for (i = 0; i < data.customer.length; i++) {
                    //console.log(data.data[i].LATITUD.replace(",", "."))
                    // console.log(data.data[i].LONGITUD.replace(",", "."))
                   /*var directionsGroup = L.marker(new L.LatLng(data.customer[0], data.customer[1]), {
                        icon: L.mapquest.icons.marker()
                    });*/
                  //  marker.bindPopup(content);
                    //directionsGroupMarkers.addLayer(marker);
                    ...
                    L.marker([data.customer[i].LATITUD.replace(",", "."), data.customer[i].LONGITUD.replace(",", ".")], {
                        icon: L.mapquest.icons.marker({
                            primaryColor: '#008f39',
                        ...
                        /*new maps.Marker({
                        position: new maps.LatLng(data.customer[i].LATITUD.replace(",", "."), data.customer[i].LONGITUD.replace(",", ".")),
                        map: map*/
                   // });
                   // let content = buildModalCustomer(data.customer[i].CODIGO, data.customer[i].COMERCIO, data.customer[i].DIRECCION, data.customer[i].TELEFONO, data.customer[i].PERSONA_CONTACTO);
                   // makeInfoWindowEvent(map, infowindow, content, marker);
                   // CalculateRouteEvent(map, marker, directionsService, directionsDisplay)
                    //Markers.push(marker);
...
/* -------------------- Direcciones de Ventas -------------------------  */
                ...
                for (i = 0; i < data.oufofroute.length; i++) {
                   /* let marker2 = new maps.Marker({
                        position: new maps.LatLng(data.oufofroute[i].cuslatitude.replace(",", "."), data.oufofroute[i].cuslongitude.replace(",", ".")),
                        map: map,
                        icon: '../../Content/img/yellow-location.png'
                    });*/
                   ...
                   /* makeInfoWindowEvent(map, infowindow, content, marker2);
                    CalculateRouteEvent(map, marker2, directionsService, directionsDisplay)*/
                    }
